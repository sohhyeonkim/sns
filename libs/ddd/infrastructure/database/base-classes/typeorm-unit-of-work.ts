...
import type { Err, Result } from 'oxide.ts/dist';
import type {
  EntityTarget,
  ObjectLiteral,
  QueryRunner,
...
import type { IUnitOfWorkPort } from '../../../domain/ports/unit-of-work.port';
...
export class TypeormUnitOfWork implements IUnitOfWorkPort {
  ...
  private async rollbackTransaction<T>(correlationId: string, error: Error) {
    ...
    try {
      ...
      this.logger.debug?.(`[Transaction rolled back] ${error.message}`);
    } finally {
      await this.finish(correlationId);
    }
  }
